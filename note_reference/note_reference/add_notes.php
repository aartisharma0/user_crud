...
if(isset($_REQUEST['add'])){
    ...
    if(isset($_FILES['pdf'])){
        ...
        $res=mysqli_query($conn,$query);
        if($res>0){
            echo "<script>window.location.assign('add_notes.php?msg=Notes Added Successfully')</script>";
        }
        else{
            echo "<script>window.location.assign('add_notes.php?msg=Error While Adding Course!')</script>";
        }
    }
