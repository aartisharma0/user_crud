...
if(isset($_REQUEST['add'])){
    ...
    if(isset($_FILES['pdf'])){
        ...
        $res=mysqli_query($conn,$query);
        if($res>0){
            echo "<script>window.location.assign('view_notes.php?msg=Note Updated Successfully')</script>";
        }
        else{
            echo "<script>console.log($res)</script>";
            echo "<script>window.location.assign('view_notes.php?msg=Something Went Wrong!!')</script>";
        }
    }
}
